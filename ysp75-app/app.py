...
import app_extensions  # נניח שקובץ ההרחבה נקרא app_extensions.py
...
def run_player_search():
    ...
    if player_query and not matching_players.empty:
        ...
        # קישור דינמי ל-Transfermarkt
        transfermarkt_link = app_extensions.generate_transfermarkt_link(selected_player)
        st.markdown(f"[🔗 חפש את השחקן ב-Transfermarkt]({transfermarkt_link})", unsafe_allow_html=True)
        ...
        if selected_club:
            ...
            # כפתור לחישוב מדד התאמה כולל שווי ידני
            if st.button("חשב מדד התאמה (Fit Score) עם שווי שוק ידני"):
                ...
                st.info("הזן שווי שוק ולחץ על כפתור 'חשב מדד התאמה' כדי לקבל תוצאה מדויקת.")

            # הצגת 10 המועדונים המובילים
            st.markdown("---")
