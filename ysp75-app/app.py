...
import os
import pandas as pd
import app_extensions  # הקובץ החדש עם הפונקציות
...
def run_player_search():
    ...
    if player_query and not matching_players.empty:
        ...
        # הצגת קישור Transfermarkt
        player_link = row.get("TransfermarktURL", None)
        app_extensions.display_transfermarkt_link(row["Player"], player_link)

        # הזנת שווי שוק ידנית לפני חישוב הפוטנציאל
        manual_value = app_extensions.market_value_section(row["Player"])

        ysp_score = app_extensions.calculate_fit_score(row, None, manual_market_value=manual_value)
        st.metric("מדד YSP-75 (פוטנציאל כולל התאמה)", ysp_score)
